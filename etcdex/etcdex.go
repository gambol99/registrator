...
func init() {
	bridge.Register(new(Factory), "etcdex")
}
...
func (r *EtcdAdapter) Register(service *bridge.Service) error {
	...
	// step: we add the attributes
	if service.Attrs == nil {
		service.Attrs = make(map[string]string, 0)
	}
	container := service.Origin.GetContainer()
	service.Attrs["image"] = container.Image

	// step: encode the document
	content, err := r.encode(service)
	if err != nil {
		return err
	}

	if r.client != nil {
		_, err = r.client.Set(path, content, uint64(service.TTL))
	} else {
		_, err = r.client2.Set(path, content, uint64(service.TTL))
	}
...
func (r *EtcdAdapter) encode(service *bridge.Service) (string, error) {
